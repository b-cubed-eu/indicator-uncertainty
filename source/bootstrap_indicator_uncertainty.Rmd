...
```{r}
# Add clasification based on reference and thresholds
insect_eveness_effects <- insect_eveness_df

insect_eveness_effects$effect <- classification(
  lcl = insect_eveness_effects$ll,
  ucl = insect_eveness_effects$ul,
  threshold = 0.1,
  reference = 0.6)

insect_eveness_effects$effect_coarse <- coarse_classification(
  insect_eveness_effects$effect)

# Create ordered factors of effects
insect_eveness_effects <- insect_eveness_effects %>%
  mutate(
    effect = case_when(
