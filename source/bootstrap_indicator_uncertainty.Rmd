...
source(here("source", "R", "leave_one_species_out_cv.R"))
...
```{r}
cv_results_loo <- leave_one_species_out_cv(
  data_cube = insect_data_new,
  fun = b3gbi::pielou_evenness_ts,
  grouping_var = "year",
  crossv_method = "loo")
...
```{r}
cv_results_kfold <- leave_one_species_out_cv(
  data_cube = insect_data_new,
  fun = b3gbi::pielou_evenness_ts,
  grouping_var = "year",
  crossv_method = "kfold",
...
How do the bootstrap distribution and the intervals look like if we exclude all species with error larger than 10 % RE in any year?
...
insect_data_new2 <- process_cube(
  insect_data_new2,
  cols_occurrences = "obs",
  cols_speciesKey = "taxonKey")

# Bootstrapping
bootstrap_insect_data2 <- bootstrap_cube(
  data_cube = insect_data_new2,
  fun = b3gbi::pielou_evenness_ts,
  grouping_var = "year",
  samples = 1000,
  seed = 123)
...
# Calculate confidence intervals
ci_df2 <- get_bootstrap_ci(
  bootstrap_samples_df = bootstrap_insect_data2,
  grouping_var = "year",
  type = "perc",
  conf = 0.95,
  aggregate = TRUE)
...
# Visualisation
bias_df2 <- bootstrap_insect_data2 %>%
  distinct(year, estimate = est_original, `bootstrap estimate` = est_boot)

estimate_df2 <- bias_df2 %>%
  pivot_longer(cols = c("estimate", "bootstrap estimate"),
               names_to = "Legend", values_to = "value") %>%
  mutate(Legend = factor(Legend, levels = c("estimate", "bootstrap estimate"),
                         ordered = TRUE))
...
bootstrap_insect_data2 %>%
  ggplot(aes(x = year)) +
    ...
    geom_point(data = estimate_df2, aes(y = value, shape = Legend),
               colour = "firebrick", size = 3, alpha = 0.5) +
    geom_errorbar(data = ci_df2, aes(ymin = ll, ymax = ul), linewidth = 1, width = 0.4,
                  colour = "cornflowerblue") +
    labs(y = "evenness",) +
    theme(legend.position = "bottom") +
    scale_y_continuous(limits = c(0, 1), breaks = seq(-10, 10, 0.25)) +
    scale_x_continuous(breaks = sort(unique(bootstrap_insect_data2$year))) +
    ggtitle("Without outliers 10 % RE")
```

We see that the eveness increases as expected without outliers.
