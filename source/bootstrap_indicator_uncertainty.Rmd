...
Like Leave-One-Out Cross-Validation (LOOCV), but where we leave out one species each time.

```{r}
data_cube_processed <- b3gbi::process_cube(
  insect_data_new,
  cols_occurrences = "obs",
  cols_speciesKey = "taxonKey")

cv_results <- leave_one_species_out_ts(
  data_cube_df = data_cube_processed,
...
ggplot(cv_results,
  ...
  geom_label(aes(y = rmse*2.5, label = "RMSE*2.5"), colour = "black", size = 3,
            nudge_x = 0.05) +
