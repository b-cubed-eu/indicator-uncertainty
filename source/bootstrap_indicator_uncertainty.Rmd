...
p <- bootstrap_insect_data_final %>%
  ggplot(aes(x = year)) +
    ...
    geom_point(data = estimate_df,
               aes(y = value, shape = Legend), colour = "firebrick", size = 3) +
...
How do the bootstrap distribution and the intervals look like if we exclude all species with error larger than `2.5*RMSE`?
...
extreme_species <- cv_results_loo %>%
  mutate(exclude = abs_diff > 2.5 * rmse) %>%
  filter(exclude) %>%
...
We see that the shape of the distributions is more symmetric and unimodal, although we still see bimodal distributions in 2015 and 2018. The different types of intervals seem to be more in accordance than before.

We also see that interval width increases with bootstrap standard error as expected.
