...
source(here("source", "scripts", "boot_statistic_diff.R"))
source(here("source", "scripts", "perform_bootstrap_ts.R"))
source(here("source", "scripts", "bootstrap_list_to_df.R"))
source(here("source", "scripts", "get_bootstrap_ci.R"))
source(here("source", "scripts", "add_classification_as_factor.R"))
...
In case of timeseries, we often want to compare values of certain years with a reference year, e.g. the first year of the series.
The variance of a difference between random variables is the sum of their variances.
However, in case of our bootstrapping examples, we cannot use the variance directly to calculate most confidence intervals of the difference.
For the normal interval, the standard error will become the square root of the sum of the bootstrap variances of two years we want to compare.
But this interval is only valid in case of symmetrical, unbiased bootstrap distributions.
...
# Summarise in dataframe
bootstrap_insect_data_diff_full <- bootstrap_list_to_df(bootstrap_insect_data_diff)
```
...
insect_eveness_diff_effects %>%
  ggplot(aes(x = year)) +
    ...
    scale_colour_manual(values =  c("darkgreen",
                                    ...
                                    "grey55"),
                        drop = FALSE) +
    labs(y = "difference in evenness compared to 2011", colour = "Classification") +
