...
break_points <- c(10, seq(25, max(cv_results_loo$perc_diff, na.rm = TRUE) + 25, 25))
...
hline_data <- data.frame(perc_diff_category = levels(cv_results_loo$perc_diff_category)) %>%
  mutate(yintercept = as.numeric(gsub("(^\\D*|,.*)", "", perc_diff_category)))
