...
incidence_estimator <- function(data, index) {
  ...
  a1_df <- data %>%
    count(taxonKey, year) %>%
    filter(n == 1) %>%
    count(n, year, name = "a1") %>%
    select(-n)
  a2_df <- data %>%
    count(taxonKey, year) %>%
    filter(n == 2) %>%
    count(n, year, name = "a2") %>%
    select(-n)
  
  summary_df <- data %>%
    summarise(
      s0 = n_distinct(taxonKey),
      N = n_distinct(cellCode),
      .by = .data$year) %>%
    left_join(a1_df, by = join_by(year)) %>%
    left_join(a2_df, by = join_by(year))
  ...
  if (index == "chao") {
    ...
    p_df <- data %>%
      mutate(N = n_distinct(cellCode),
             .by = .data$year) %>%
      count(taxonKey, year, N) %>%
      mutate(p = n / N,
             calc = (1 - .data$p)^.data$N) %>%
      summarise(
        sum_of_calc = sum(calc),
        .by = .data$year
      )
    
    out_df <- summary_df %>%
      left_join(p_df, by = join_by(year)) %>%
      mutate(
